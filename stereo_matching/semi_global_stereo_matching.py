...
rect_left_image = cv2.imread("im1_left.png")  # 輸入經過相機標定的照片
...
disparity_norm = cv2.normalize(disparity,
                               disparity,
                               alpha=0,
                               beta=255,
...
filtered_disp = wls_filter.filter(disparity, rect_left_image, None,
                                  rect_right_image)
filtered_disp_norm = cv2.normalize(filtered_disp,
                                   filtered_disp,
                                   alpha=0,
                                   beta=255,
                                   norm_type=cv2.NORM_MINMAX,
...
cv2.imwrite(
    'disparity.jpg',
    cv2.resize(disparity_norm,
               (disparity_norm.shape[1] // 2, disparity_norm.shape[0] // 2)))
cv2.imwrite(
    'filtered_disparity.jpg',
    cv2.resize(
        filtered_disp_norm,
        (filtered_disp_norm.shape[1] // 2, filtered_disp_norm.shape[0] // 2)))